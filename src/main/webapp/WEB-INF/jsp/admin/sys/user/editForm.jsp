...
<div class="panel">
    <ul class="nav nav-tabs">
        <shiro:hasPermission name="sys:user:create">
            <c:if test="${op eq '新增'}">
                <li ${op eq '新增' ? 'class="active"' : ''}>
                    <a href="${ctx}/admin/sys/user/create?BackURL=<doshou:backURL/>">
        ...
        <c:if test="${not empty m.id}">
            <li ${op eq '查看' ? 'class="active"' : ''}>
                <a href="${ctx}/admin/sys/user/${m.id}?BackURL=<doshou:backURL/>">
                    <i class="fa fa-eye fa-fw"></i> 查看
                </a>
            </li>
            <shiro:hasPermission name="sys:user:update">
                <li ${op eq '修改' ? 'class="active"' : ''}>
                    <a href="${ctx}/admin/sys/user/${m.id}/update?BackURL=<doshou:backURL/>">
            ...
            <shiro:hasPermission name="sys:user:delete">
                <li ${op eq '删除' ? 'class="active"' : ''}>
                    <a href="${ctx}/admin/sys/user/${m.id}/delete?BackURL=<doshou:backURL/>">
    ...
    <form:form id="editForm" method="post" commandName="m" cssClass="form-inline form-horizontal form-small form-wrapper">
        ...
        <c:if test="${op eq '删除'}">
            <c:set var="icon" value="fa-trash-o"/>
        </c:if>
