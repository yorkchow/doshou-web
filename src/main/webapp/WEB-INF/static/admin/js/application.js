...
$.app = {
    ...
    waiting : function(message, isSmall) {
        ...
        message = '<img src="' + ctx + '/static/admin/images/loading.gif" '+ (isSmall ? "width='20px'" : "") +'/> ' + message;
        if(!isSmall) {
...
//            });
    ...
    initDatetimePicker : function() {
        ...
        $('.date:not(.custom)').each(function() {
            ...
            $date.find(":input").click(function() {$date.find(".fa-calendar,.fa-clock-o,.icon-date").click();});
    ...
    toggleLoadTable : function(tableEle, asyncLoadURL) {
        var openIcon = "fa-plus-circle";
        var closeIcon = "fa-minus-circle";
    ...
    initSort: function (table) {
        ...
        table.find("[sort]").each(function () {
            ...
            if (matchResult) {
                order = RegExp.$1;
                if (order == 'asc') {
                    sortBtnStr = sortBtnTemplate.replace("{sort-icon}", "sort-hover fa fa-caret-up");
                } else if (order == 'desc') {
                    sortBtnStr = sortBtnTemplate.replace("{sort-icon}", "sort-hover fa fa-caret-down");
                }
            }
            if (sortBtnStr == null) {
                sortBtnStr = sortBtnTemplate.replace("{sort-icon}", "fa fa-caret-down");
            }
